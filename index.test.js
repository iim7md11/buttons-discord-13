...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.startsWith('o')) {
        ...
        let group1 = new MessageActionRow().addComponents(btn);
        let group2 = new MessageActionRow().addComponents(select);
        let m = await message.channel.send({ content: 'hi', components: [group1, group2] });
    }
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        interaction.deferUpdate().then(async (r) => {
            await wait(1000);
            r.editReply({ content: 'awaited', components: [btn] });
        });
    }
    ...
    if (interaction.isSelectMenu()) {
        if (interaction.values[0] === 'reload') {
            ...
            interaction.message.edit({ content: 'hey', components: interaction.message.components });
        }
    }
});
