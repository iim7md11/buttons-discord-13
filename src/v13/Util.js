const { MessageActionRow, MessageButton, MessageSelectMenu, MessageComponentTypes } = require('discord.js');
...
module.exports = {
  ...
  resolveButton(data) {
    ...
    if (data.style === 'LINK' && !data.url) throw new TypeError('NO_BUTTON_URL: You provided url style, you must provide an URL');

    if (data.style !== 'LINK' && data.url)
      throw new TypeError('BOTH_URL_CUSTOM_ID: A custom id and url cannot both be specified');

    if (data.style === 'LINK' && data.custom_id)
      throw new TypeError('BOTH_URL_CUSTOM_ID: A custom id and url cannot both be specified');

    if (data.style !== 'LINK' && !data.custom_id) throw new TypeError('NO_BUTTON_ID: Please provide button id');
